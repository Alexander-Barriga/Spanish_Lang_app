...
BEGIN 
    ...
    -- ============================================
    -- is_premium: cached server-side entitlement flag. Updated by RC webhook or comp redemption.
    ...
    END IF;
    -- subscription_tier: 'free' | 'monthly' | 'annual' | 'comp'
    ...
    END IF;
    -- subscription_expires_at: NULL for comp/free or never-expiring entitlements; set for paid auto-renewing.
    ...
    END IF;
    -- entitlement_source: provenance of the current entitlement.
    ...
    END IF;
    -- revenuecat_app_user_id: stable identifier we sent to RC (mirrors users.id today, but keep separate for flexibility).
...
-- ============================================
-- We store ONLY a SHA-256 hash of the code; plaintext is shown once at generation.
...
-- Per-user redemption ledger; prevents the same user redeeming the same code twice
-- and gives us an audit trail.
